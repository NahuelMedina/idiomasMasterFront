...
if(location.pathname === '/cart'){
  return (
    ...
    ( <div className="">
      { 
            isFav  ? (
               <button onClick={handleFavorite} className=" absolute top-2 right-[370px] text-2xl ">❤️</button>
            ) : (
               <button onClick={handleFavorite} className=" absolute top-2 right-[370px] text-2xl ">🤍</button>
            )
         } 
      </div>)}
      {location.pathname === '/favorite' && (
      <div>
        <button onClick={handleRemoveFromFavorites} className=" absolute top-2 right-2 text-3xl " ><RxCrossCircled className="bg-white rounded-[15px]" /></button>
      </div>)}
     ...
     { isCart ? (
            ...
            <div className="flex">
                ...
                <button onClick={handleCart}  className="text-black text-[25px] ">Agregar al carrito</button>
            </div>)
      }
      </div>
...
return (
    ...
    ( <div className="">
      ...
      <Link to={`/detail/${course._id}`} className="bg-[#FF6B6C] w-full h-[40px] flex flex-row items-center justify-center hover:bg-yellow-500" >
